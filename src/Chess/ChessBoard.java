...
public class ChessBoard {
    ...
    public ChessBoard(String nowPlayer) { // строковая переменная текущего игрока
    ...
    // с какой позиции на какую позицию нужно перенести фигуру
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        // проверяем, что такая клетка существует на доске
        if (checkPos(startLine) && checkPos(startColumn)) {

            // если цвет текущего игрока не совпадает с цветом фигуры на данной клетке - ход невозможен
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                ...
                board[endLine][endColumn] = board[startLine][startColumn]; // фигуру со старт клетки на конеч клетку
                board[startLine][startColumn] = null; // удаляем фигуру со стартовой клетки
                this.nowPlayer = this.nowPlayerColor().equals("White") ? "Black" : "White"; // если был ход белых -
                // следующий ход черных и наоборот
